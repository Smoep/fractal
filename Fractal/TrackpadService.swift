...
private let fractalLog = Logger(subsystem: "com.jos.fractal", category: "trackpad")
...
final class TrackpadService {
    ...
    func start() {
        ...
        fractalLog.info("Started — MT devices: \(self.mtDevices.count), global monitors: ok")
    }
    ...
    func handleMultitouch(fingerCount: Int32, touchPosition: CGPoint? = nil, size: Float = 0, density: Float = 0) {
        ...
        if prev == 0 && fingerCount == 1 && !fingerDown {
            ...
            fractalLog.info("MT down 0→1: trigger=\(trigger.rawValue, privacy: .public)")
        }
        // N → 0: all fingers lifted.
            ...
            if isEngaged {
                ...
                fractalLog.info("MT up →0 while engaged: liftMode=\(liftMode, privacy: .public) justEngaged=\(self.justEngaged, privacy: .public) → \(liftMode ? "pendingClick(finalize)" : "clickToSelect", privacy: .public)")
                if liftMode {
    ...
    private func handleGlobalEvent(_ event: NSEvent) {
        ...
        case .leftMouseDown:
            if isEngaged { return }
            let trigger = settings?.activationTrigger ?? .tapToClick
            fractalLog.info("leftMouseDown recv: trigger=\(trigger.rawValue, privacy: .public) size=\(self.lastFingerSize, privacy: .public)/peak\(self.peakFingerSize, privacy: .public)")
            if trigger == .click {
                if lastFingerSize < clickFingerPresenceThreshold {
                    fractalLog.info("leftMouseDown ignored — tap, no finger contact (size=\(self.lastFingerSize, privacy: .public))")
                    return
                }
                if shouldSuppressActivation?() != true, !isMouseOverOwnWindow(),
                   settings?.trackpadEnabled != false {
                    ...
                    startHoldTimer()
                    fractalLog.info("leftMouseDown → click hold started (\(holdDur, privacy: .public)s)")
                }
                return
            }
    ...
    private func engage() {
        ...
        fractalLog.info("ENGAGED — overlay opening")
        candidateOverlay.hide()
        isTouching = true
        isEngaged = true
        justEngaged = true
