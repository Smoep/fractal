...
struct ContentView: View {
    ...
    @State private var menuBackupAlert: MenuBackupAlert?
    ...
    var body: some View {
        ...
        .onReceive(NotificationCenter.default.publisher(for: .fractalToggleTracking)) { _ in
            if engine.isRunning { engine.stop() } else { engine.start() }
        }
        .alert(item: $menuBackupAlert) { alert in
    ...
    @ViewBuilder private var menuTab: some View {
        SettingsSection(title: "Radial Menu") {
            ...
            HStack(spacing: 12) {
                ...
                Button(action: importRadialMenu) {
                    Label("Import", systemImage: "square.and.arrow.down")
                }
                .help("Import a Fractal menu backup")

                Button(action: exportRadialMenu) {
                    Label("Export", systemImage: "square.and.arrow.up")
                }
                .help("Export the current radial menu")
            }
            .padding(.top, 4)
        }
    }

    private func exportRadialMenu() {
        let panel = NSSavePanel()
        panel.title = "Export Radial Menu"
        panel.nameFieldStringValue = "Fractal Menu Backup.json"
        ...
        do {
            let data = try RadialMenuStore.shared.exportBackupData()
            try data.write(to: url, options: .atomic)
            menuBackupAlert = MenuBackupAlert(
                title: "Menu Exported",
                message: "Your Fractal menu was saved to \(url.lastPathComponent)."
            )
        } catch {
            menuBackupAlert = MenuBackupAlert(
                title: "Export Failed",
    ...
    private func importRadialMenu() {
        let panel = NSOpenPanel()
        panel.title = "Import Radial Menu"
        panel.allowedContentTypes = [.json]
        ...
        do {
            ...
            try RadialMenuStore.shared.importBackupData(data)
            menuBackupAlert = MenuBackupAlert(
                title: "Menu Imported",
                message: "The radial menu was restored from \(url.lastPathComponent)."
            )
        } catch {
            menuBackupAlert = MenuBackupAlert(
                title: "Import Failed",
    ...
    @ViewBuilder private var appearanceTab: some View {
        SettingsSection(title: "Menu Appearance") {
            SettingsSlider("Branch Length",
                value: Binding(get: { engine.settings.ringHeight },
                               set: { engine.settings.ringHeight = $0 }),
                range: 60...200, step: 10, format: "%.0f pt",
                caption: "Length of each branch segment")

            Divider().padding(.vertical, 2)

            SettingsSlider("Branch Spacing",
                value: Binding(get: { engine.settings.selectionWidth },
                               set: { engine.settings.selectionWidth = $0 }),
                range: 34...80, step: 2, format: "%.0f pt",
                caption: "Gap between sibling nodes (wider = looser fan)")
            ...
            SettingsSlider("Backdrop Spread",
                value: Binding(get: { engine.settings.overlayBackdropSpread * 100 },
                               set: { engine.settings.overlayBackdropSpread = $0 / 100 }),
                range: 0...100, step: 5, format: "%.0f%%",
                caption: "Width of the soft blur field around branches and labels")

            Divider().padding(.vertical, 2)

            SettingsSlider("Backdrop Intensity",
                value: Binding(get: { engine.settings.overlayBackdropIntensity * 100 },
                               set: { engine.settings.overlayBackdropIntensity = $0 / 100 }),
                range: 0...100, step: 5, format: "%.0f%%",
                caption: "Strength of the background separation behind the menu")
        }
    }
...
private struct MenuBackupAlert: Identifiable {
    let id = UUID()
    let title: String
    let message: String
}
