...
/// Versioned, portable representation of the Radial Menu configuration.
struct RadialMenuBackup: Codable {
    static let currentVersion = 1
    static let formatIdentifier = "com.jos.fractal.menu-backup"
    static let legacyFormatIdentifier = "com.jos.radial-tree.menu-backup"

    let format: String
    let version: Int
    let exportedAt: Date
    let categories: [RadialCategory]

    init(categories: [RadialCategory], exportedAt: Date = Date()) {
...
enum RadialMenuBackupError: LocalizedError {
    case invalidFile
    ...
    var errorDescription: String? {
        switch self {
        case .invalidFile:
            "This is not a valid Fractal menu backup."
        case .unsupportedVersion(let version):
            "This menu backup uses unsupported format version \(version)."
        case .emptyMenu:
            "The menu backup does not contain any categories."
        }
    }
}

enum RadialMenuBackupCodec {
    static func encode(categories: [RadialCategory], exportedAt: Date = Date()) throws -> Data {
        let encoder = JSONEncoder()
        encoder.dateEncodingStrategy = .iso8601
        encoder.outputFormatting = [.prettyPrinted, .sortedKeys, .withoutEscapingSlashes]
        return try encoder.encode(RadialMenuBackup(categories: categories, exportedAt: exportedAt))
    }

    static func decode(_ data: Data) throws -> [RadialCategory] {
        let decoder = JSONDecoder()
        decoder.dateDecodingStrategy = .iso8601

        let backup: RadialMenuBackup
        do {
            backup = try decoder.decode(RadialMenuBackup.self, from: data)
        } catch {
            throw RadialMenuBackupError.invalidFile
        }

        guard backup.format == RadialMenuBackup.formatIdentifier
                || backup.format == RadialMenuBackup.legacyFormatIdentifier else {
            throw RadialMenuBackupError.invalidFile
        }
        guard backup.version == RadialMenuBackup.currentVersion else {
            throw RadialMenuBackupError.unsupportedVersion(backup.version)
        }
        guard !backup.categories.isEmpty else {
            throw RadialMenuBackupError.emptyMenu
        }
...
final class RadialMenuStore {
    ...
    static let shared: RadialMenuStore = {
        FractalDefaultsMigration.run()
        return RadialMenuStore()
    }()
    ...
    /// Encode the current menu as a versioned JSON backup.
    func exportBackupData() throws -> Data {
        try RadialMenuBackupCodec.encode(categories: categories)
    }

    /// Validate and replace the current menu from a versioned JSON backup.
    func importBackupData(_ data: Data) throws {
        categories = try RadialMenuBackupCodec.decode(data)
    }
