...
enum FractalDefaultsMigration {
    private static let sourceDomain = "com.jos.radial-tree"
    private static let markerKey = "fractalDefaultsMigratedFromRadialTree"
    private static let keys = [
        "activationHoldDuration",
        "gridDivisions",
        "dragRange",
        "ringDelay",
        "liftToSelect",
        ...
        "ringHeight",
        "selectionWidth",
        "categoryFlexibilityPercent",
        "pauseWhileTyping",
        "activationTrigger",
        "overlayOpacity",
        "overlayBackdropSpread",
        ...
        "mouseButton",
        "mouseHoldDuration",
        "mouseReleaseToSelect",
        "radialMenuCategories",
        "treeLayoutV2Migrated"
    ]
...
final class AppSettings {
    ...
    private func persistAll() {
        ...
        d.set(overlayBackdropSpread, forKey: "overlayBackdropSpread")
        d.set(overlayBackdropIntensity, forKey: "overlayBackdropIntensity")
    ...
    /// Length of each branch segment (points). Longer lines let siblings cluster
    /// tightly in the growth direction without their circles overlapping.
    var ringHeight: Double = 120 {
        didSet { scheduleSave() }
    }

    /// Center-to-center distance between adjacent sibling nodes (points).
    var selectionWidth: Double = 40 {
        didSet { scheduleSave() }
    }
    ...
    /// Width of the soft blur field around the tree (0.0–1.0).
    var overlayBackdropSpread: Double = 0.34 {
        didSet { scheduleSave() }
    }

    /// Strength of the blurred material behind the tree (0.0–1.0).
    var overlayBackdropIntensity: Double = 0.28 {
        didSet { scheduleSave() }
    }
    ...
    private init() {
        ...
        if let v = d.object(forKey: "overlayBackdropSpread") as? Double {
            overlayBackdropSpread = min(max(v, 0), 1)
        }
        if let v = d.object(forKey: "overlayBackdropIntensity") as? Double {
            overlayBackdropIntensity = min(max(v, 0), 1)
        }
        ...
        // One-time migration to the fractal-tree geometry: the old ring scale
        // (30–100) is too short for a tight directional fan, so reset length and
        // sibling spacing to the new defaults once. Later user changes are kept.
        if d.object(forKey: "treeLayoutV2Migrated") == nil {
            ringHeight = 120
            selectionWidth = 40
            d.set(true, forKey: "treeLayoutV2Migrated")
        }
    }
}
